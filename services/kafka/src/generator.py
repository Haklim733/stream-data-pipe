# source: https://github.com/jaehyeon-kim/flink-demos/blob/master/building-pyflink-apps/src/s05_data_gen.py
...
if __name__ == "__main__":
    ...
    parser.set_defaults(partitions=1)
    parser.add_argument("--replication_factor")
    parser.set_defaults(replication_factor=1)
    args = parser.parse_args()

    client = KafkaClient("localhost:19092")
    if args.create:
        topics = [
            NewTopic(name="skyone", num_partitions=args.partition, replication_factor=args.replication_factor),
            NewTopic(name="sunset", num_partitions=args.partition, replication_factor=args.replication_factor),
            NewTopic(name="flightdata", num_partitions=args.partition, replication_factor=args.replication_factor),
            NewTopic(name="userstatistics", num_partitions=args.partition, replication_factor=args.replication_factor),
        ]
